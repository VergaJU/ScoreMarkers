...
class DefineLabel:
    ...
    def score_label(self, alpha=1, beta=1):
        ...
        for celltype in celltypes:
            ...
            cells["pos"] = (cells["pos"] - cells["pos"].min()) / (
                        cells["pos"].max() - cells["pos"].min())  # minmax norm
            count = 0
            ...
            cells["neg"] = (cells["neg"] - cells["neg"].min()) / (
                        cells["neg"].max() - cells["neg"].min())  # minmax norm

            cells[celltype] = (alpha * cells["pos"]) - (
                        beta * cells["neg"])  # get final score summing pos and neg by bias
